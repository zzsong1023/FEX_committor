...
def sampling(N, N_bound, dim, b, T, r):
    beta = 1/T
    ct = 1
    tmp = np.zeros((1,1)) #[[0.]]
    Xmid = tmp
    while ct<N:
        tmp = 2*(-0.5+np.random.rand(1,1)) # tmp is a random number between (-1,1)
        Xmid = np.vstack((Xmid,tmp))
        ct   = ct+1


    Xmid = np.hstack((Xmid, np.sqrt(T/2./b)*np.random.randn(N,dim-1))) 
    ...
    normal = lambda x: np.exp(-beta*(x**2-1)**2)
    normal_const = 1/quad(normal,-1,1)[0]   
    yA[0:N_bound,:]              = 1
    yB[N_bound:2*N_bound,:]      = 1
    yin[2*N_bound:2*N_bound+N,0] = np.exp(-1.0/T*(Xmid[:,0]**2 - 1**2)**2)*normal_const
...
def train_controller(Xin,Yin,controller, controller_optim, trainable_tree, tree_params, hyperparams):
    ...
    for candidate_ in candidates.candidates:
        ...
        Ntest = 1*10**5
        Xtest, Ytest = sampling(Ntest, int(Ntest*args.alpha), dim, args.b, args.T, args.r)

        Xtest = torch.from_numpy(Xtest).to(get_device()).float()
        qtest = trainable_tree(Xtest, candidate_.action)
        qtest = qtest.cpu().detach().numpy()
        rtest = Xtest[:,0]
        qpred = func_com.Wq(rtest, 1/args.T, args.r)
        
        err = np.linalg.norm(qtest-qpred)/np.linalg.norm(qpred)

        logger.append(['relative_l2', 0, 0, 0, err, 0])
...
if __name__ == '__main__':
    ...
    Xin, Yin = sampling(args.N, args.N_bound, dim, args.b, args.T, args.r)
